...
public class QueryStringLinkAttrProcessor extends AbstractAttributeTagProcessor {
  ...
  protected void doProcess(ITemplateContext context, IProcessableElementTag tag, AttributeName attributeName // thymeleaf
    ...
    final RequestContext requestContext = (RequestContext) context
        .getVariable(SpringContextVariableNames.SPRING_REQUEST_CONTEXT); // URI를 구하기 위한 Request Context
    ...
    if (StringUtils.isEmptyOrWhitespace(attributeValue)) { // queryString 속성에 값이 없을 경우 null로 return 한다
      ...
      // 예를 들어 attributeValue에 @{/board/ist.html(page=1)가 있으면 이를 LinkExpression 객체로
      // 변환할 경우 /board/list.html 과 page=1을 각각 분리해서 보관한다
      // 즉 LinkExpression 객체의 base 속성엔 /board/list.html을, parameters 속성엔 page=1을 갖게 된다
      linkExpression = (LinkExpression) StandardExpressions.getExpressionParser(context.getConfiguration())
          .parseExpression(context, attributeValue);

      if (linkExpression == null) {
        expressionResult = null;
      } else {
        if (requestContext.getQueryString() == null) { // Request Cnntext에서 QueryString이 없으면 QueryString을 붙일 필요가 없어지기
                                                       // 때문에 사용자가 queryString 속성에 입력한 값만을 가지고 작업하면 된다
          ...
          // 사용자가 queryString 속성에 넣은 값에 파라미터가 있을 경우 QueryString 에서 사용자가 넣은 파라미터와 동일한 값이
          // 있는지 찾아보고
          // 이것이 있을 경우 이를 제거한뒤에 사용자가 queryString 속성에 넣은 파라미터 값으로 대체해서 넣는다
          // 예를 들어 Request QueryString에 check=name&page=3이란 값이 있고 queryString 속성에 넣은 값으로
          // @{/board/list.html(page=1)} 이렇게 있으면
          // page=3과 page=1이 같은 파라미터 이름으로 중복되기 때문에 page=3을 지우고 page=1로 대체하는 작업을 진행한다
          // 대체하는 방법은 해당 파라미터를 key로 하고 있는 NameValuePair 객체를 List에서 찾아서 이를 지운뒤에 page=1을
          // NameValuePair 객체로 생성해서
          // 이를 List에 넣는다
          if (assignationSequence != null) {
            for (Assignation assignation : assignationSequence) {
              ...
              nvp.add(new BasicNameValuePair(assignation.getLeft().getStringRepresentation(),
                  assignation.getRight().getStringRepresentation()));
            }
          }

          // 최종 작업이 완료된 QueryString 이 들어있는 List 객체안의 값들을 key=value 형태의 문자열로 만든뒤에 이것을 각각 ,로
          // 결합한다
          // 위에서 사용한 예를 가지고 표현하게 되면 check=name,page=1 이렇게 된다
          // 이렇게 만든 문자열을 이따가 Thymeleaf에서 queryString 속성에 넣은 값의 파라미터로 다시 재구성을 하게 된다
          // 즉 @{/board/list.html(page=1)}을 @{/board/list.html(check=name,page=1)} 으로 바꾸는
          // 것이다
          // 아래의 expressionResult 까지가 그 과정이다
          final String parameters = nvp.stream().map(nv -> nv.getName() + "=" + nv.getValue() + "")
          ...
          stringBuilder.append(PARAMS_START_CHAR).append(parameters).append(PARAMS_END_CHAR)
              .append(EXPRESSION_END_CHAR);

          attributeValue = stringBuilder.toString();
          expressionResult = EngineEventUtils.computeAttributeExpression(context, tag, attributeName, attributeValue)
              .execute(context);
        }
      }
    }

    // 최종 작업이 완료된 expressionResult 변수에 저장된 값을 이용해서 href 속성에 HTML Link 형태의 값으로 변환한 값을
    // 설정하게 된다
