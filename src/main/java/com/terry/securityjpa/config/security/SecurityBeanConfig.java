...
public class SecurityBeanConfig {
  ...
  /**
   * 기본적으로 설정되는 DaoAuthenticationProvider는 UserNotFoundException이 던져진것을 받게 될 경우
   * 이를 BadCridentialsException으로 변환해서 던지게 된다.
   * 그 이유는 DaoAuthenticationProvider 멤버변수로 hideUserNotFoundExceptions가 있는데 이 변수값이 true일 경우
   * UserNotFoundException을 받을때 이를 BadCridentialsException으로 변환하기 때문이다.
   * 이 변수의 기본값이 true이기 때문에 UserNotFoundException을 던져도 최종적으로 받게 되는 예외는 BadCridentialsException이 된다.
   * 그래서 UserNotFoundException을 던질경우 이것을 그대로 UserNotFoundException으로 받기 위해
   * DaoAuthenticationProvider bean을 새로 설정한 뒤 여기에 userDetailService와 passwordEncoder를 설정한뒤
   * hideUserNotFoundExceptions를 false로 설정해서
   * 이를 WebSecurityConfigurerAdapter 클래스의 configure(AuthenticationManagerBuilder auth)에서 사용하도록 했다
   * @return
   */
  @Bean
  ...
  /**
   * Spring Security에서 세션 동시 접속 제어를 설정할 경우 HttpSessionEventPublisher 클래스가 web.xml에 listener로 등록되어
   * 야한다.
   * Spring Boot에서는 @Bean으로 Spring Bean을 설정하는 것으로 listener로 등록이 된다. 그러나 이러한 동작 방식은 Spring Boot의
   * Embedded Container에 제한된 것이지, war 파일을 통한 배포에서는 맞지 않을수 있어서 이 부분에 대한 확인이 필요하다
   * @return
   */
  @Bean
  ...
  public AffirmativeBased accessDecisionManager(CacheService cacheService, RoleHierarchy roleHierarchy) {
    ...
    // CustomAuthorityVoter customAuthorityVoter = new CustomAuthorityVoter(cacheService);
    CustomAuthorityVoter customAuthorityVoter = new CustomAuthorityVoter();
    AuthenticatedVoter authenticatedVoter = new AuthenticatedVoter();
    List<AccessDecisionVoter<? extends Object>> accessDecisionVoterList = Arrays.asList(customRoleVoter, roleHierarchyVoter, customAuthorityVoter, authenticatedVoter);
    AffirmativeBased accessDecisionManager = new AffirmativeBased(accessDecisionVoterList);
    // 접근 승인이나 거부에 대한 판단을 할 수 없는 경우 접근 허용 여부를 설정하는 것이 allowIfAllAbstainDecisions 속성인데 이것을 true로 하면 접근을 허용하는 것이고 그렇지 않은 경우 접근을 허용하지 않는 것을 의미한다
  ...
  public CustomFilterSecurityInterceptor customFilterSecurityInterceptor(AuthenticationManager authenticationManager
    ...
    CustomFilterSecurityInterceptor customFilterSecurityInterceptor = new CustomFilterSecurityInterceptor(permitAllPattern);
    ...
    // 그러나 현실적으로 public page가 없을 수는 없다(login.html을 예로 들면 여러 권한(익명 권한 포함)을 가진 사람이 모두 접근할 수 있는 public page 이다)
  ...
  /**
   * Role에 대한 정보를 DB에서 읽어와서 RoleHierachy를 생성한다
   * 부모 Role 이름 > 자식 Role 이름(ex AdminRole > UserRole) 형태의 문자열을 만들어 이를 RoleHierachyImpl 클래스의 setHierachy 메소드에
