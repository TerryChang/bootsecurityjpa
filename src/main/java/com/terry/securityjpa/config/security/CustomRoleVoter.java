      ...
      if (this.supports(attribute)) {
        ...
        for (String roleName : roleSet) {
          for (Role role : keySet) {
            ...
            // 로그인 한 사용자의 role과 cache에 저장되어 있는 role을 찾으면 그에 따른 권한(authority)를 가져와서 비교 작업을
            // 진행한다
            if (roleName.equals(role.getRoleName())) {
              ...
              for (GrantedAuthority authority : grantedAuthorityList) {
                if (attribute.getAttribute().equals(authority.getAuthority())) { // role에 있는 권한과 자원에 있는 권한이 같으면 접근 가능하다고
                                                                                 // 설정해준다
